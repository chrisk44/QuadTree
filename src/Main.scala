...
object Main extends App {
  ...
  def run_interface: Unit = {
    ...
    val lower_bound = readDouble("Enter lower bound for all dimensions: ")  // Suggested value: -10000.0
    val upper_bound = readDouble("Enter upper bound for all dimensions: ")  // Suggested value: 10000.0
    val tree: QuadTree = new QuadTree(dim, lower_bound, upper_bound)
        ...
        case 1 =>
          ...
          while(count>0){
            ...
            try{
              ...
              // Insertion worked, so point didn't exist and was valid, so decrease count
              count = count - 1
